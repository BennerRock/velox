package com.optima.mixin;

import com.optima.OptimaConfig;
 ...
 * {@link OptimaConfig#readOnlyLoad()} - which never creates the file and never logs. The entry
 * point loads the config again later; both paths converge on the same values because
 * {@code apply} is idempotent.</p>
 */
public final class OptimaMixinPlugin implements IMixinConfigPlugin {

    private static final String PKG_TICK = "com.optima.mixin.tick.";
    private static final String PKG_CLIENT = "com.optima.mixin.client.";
    ...
    private OptimaConfig config() {
        if (!configLoaded) {
            synchronized (this) {
                if (!configLoaded) {
                    try {
                        OptimaConfig.INSTANCE.readOnlyLoad();
