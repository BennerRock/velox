...
public final class MemoryWatchdog {
    ...
    public static synchronized void start() {
        if (!OptimaConfig.INSTANCE.memoryWatchdog || thread != null) {
            return;
        }

        int interval = Math.max(30, OptimaConfig.INSTANCE.memoryWatchdogIntervalSeconds);
        ...
        thread = new Thread(() -> {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (OptimaConfig.INSTANCE.collectStats) {
                    sb.append(" | ").append(OptimaRuntime.report());
                }

                Optima.LOGGER.info(sb.toString());

                if (pct >= 90.0) {
                    Optima.LOGGER.warn(
                            "[Optima] Heap is {}% full and stays above 90%. Raise -Xmx, or profile with a sampler - "
                                    + "at this level the JVM spends most of its time collecting instead of ticking.",
                            String.format("%.0f", pct));
                }
            }
        }, "optima-memory-watchdog");
    ...
    private static void installShutdownHook() {
        ...
        hookInstalled = true;
        try {
            Runtime.getRuntime().addShutdownHook(new Thread(MemoryWatchdog::stop, "optima-watchdog-shutdown"));
        } catch (Throwable t) {
