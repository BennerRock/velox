...
public final class PlayerProximity {
    ...
    public static double nearestPlayerDistanceSqr(Entity entity, double thresholdSq) {
        ...
        if (acc == null) {
            ...
            if (acc == null) {
                ...
                Velox.LOGGER.warn("[Velox] No way to reach a player list from {}; "
        ...
        try {
            ...
            DEAD.put(cls, Boolean.TRUE);
            Velox.LOGGER.warn("[Velox] Disabling distance optimizations for {}: {}", cls.getName(), t);
            return UNKNOWN;
        }
    }
    ...
    private static void describe(Class<?> entityClass, Method levelMethod, Method listMethod) {
        ...
        Velox.LOGGER.info("[Velox] Player proximity resolved for {}: {}",
    ...
    public static void logResolution() {
        if (lastResolvedDescription != null) {
            Velox.LOGGER.info("[Velox] Player proximity: RESOLVED via {}", lastResolvedDescription);
        } else if (!DEAD.isEmpty()) {
            Velox.LOGGER.warn("[Velox] Player proximity: UNRESOLVED - distance optimizations disabled.");
        } else {
            Velox.LOGGER.info("[Velox] Player proximity: not exercised yet (it resolves on first use).");
        }
    }
}
